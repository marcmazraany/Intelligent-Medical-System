...
public class AlertService {
    ...
    public List<AlertDto> list(UUID userId) {
        return repo.findAllByUserIdOrderByCreatedDateDesc(userId)
                .stream().map(AlertService::toDto).toList();
    }
    ...
    public AlertDto patch(UUID userId, UUID id, PatchAlertRequest req) {
        ...
        if (req.maxDistance()  != null) e.setMaxDistance(req.maxDistance());
